...
/**
 */
public class CohortDrugCollectionsReportDT implements GenerateExcelReportInterface {
	...
	private ExcelReportObjectDT reportObject;

	private List<PackageExportObjectDT> extraColumns;
	 ...
	 * @param reportObject
	 */
	public CohortDrugCollectionsReportDT(String path, ExcelReportObjectDT reportObject) {
		super();
		if (reportObject.isShowBatchInfo()) {
			sheetNAME = "Drug Issues (Incl Batch Info)";
		} else {
			sheetNAME = "Track Levantamentos";
		}
	...
	public void writeHeadings(int numextracolumns) {
		List<PackageExportObjectDT> columns = reportObject.getColumns();
		int drugColumnOffset = 0;
		for (int i = 0; i < columns.size(); i++) {

			PackageExportObjectDT drugDispensedObject = columns.get(i);
		...
		if (extraColumns != null){
			for (int i = 0; i < numextracolumns; i++) {
				for (int j = 0; j < extraColumns.size(); j++) {
					PackageExportObjectDT ddo = extraColumns.get(j);
					// ddo = ddo.clone();
	 ...
	 * @param text
	 */
	private void writeHeading(PackageExportObjectDT column) {
		int rowsToMerge = 2;
	...
	public void writeTitle() {
		...
		xlwm.writeTitleCell(1, rowCounter,
				"Cohort para Pacientes em Dispensa Trimestral");
		...
		xlwm.writeSubHeadingCellTextLeft(1, rowCounter, "Unidade Sanitaria");
		...
		xlwm.mergeCells(1, rowCounter, 4, rowCounter);
		xlwm.writeSubHeadingCellTextLeft(1, rowCounter, "Periodo");
		xlwm.mergeCells(5, rowCounter, 9, rowCounter);
		xlwm.writeSubHeadingCellTextLeft(5, rowCounter, MessageFormat.format(
				"{0,date,short} - {1,date,short}", reportObject.getStartDate(), reportObject.getEndDate()));
	...
	public void writeRow(DataExportFunctions functions) {
		List<PackageExportObjectDT> columns = reportObject.getColumns();
		for (PackageExportObjectDT column : columns) {
	...
	public void writeExtraColumns(DataExportFunctions functions, int num) {
		if (extraColumns != null){
			...
			for (int j = 0; j < extraColumns.size(); j++) {
				PackageExportObjectDT column = extraColumns.get(j);
