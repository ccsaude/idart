...
public abstract class ExcelExporterDT {

	static Logger log = Logger.getLogger(ExcelExporterDT.class);
	...
	public ExcelExporterDT() {
		super();
	}

	public void generate(ExcelReportObjectDT report) throws ReportException {
		...
		if (monitor != null){
			monitor.beginTask("Processando o Relatório", entitySet.getSize());
		}
		...
		try {
			while (!isCancelled()) {
				...
				updateMonitorMessage("Processado(s) " + page * PAGE_SIZE
						+ " Paciente(s) de " + total);
	...
	protected abstract GenerateExcelReportInterface getExcelReport(ExcelReportObjectDT report);
	...
	protected void setupFunctions(DataExportFunctions functions, ExcelReportObjectDT report) {
	...
	protected abstract EntitySet getPatientSet(ExcelReportObjectDT report);
